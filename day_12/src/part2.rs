...
pub fn run() {
  ...
  // By printing out from left_plant (PotLine.println), we see we reach
  // a stable form after about 135 generations, and extrapolate from
  // there to do the math for 50 billion generations.
  //
  // You'll need to print on a wide terminal (200 chars or more) to
  // eyeball the patterns.
...
impl PotLine {
  ...
  pub fn println(&self, t: usize) {
    print!("{:4} ", t);
    print!("{:4} ", self.left_plant);

    for i in self.left_plant ..= self.right_plant {
