...
pub fn run() {
  let mut elf_power = 3u32;
  loop {
    let mut content: Vec<u8> = Vec::new();
    content.extend_from_slice(include_bytes!("input.txt"));
    let mut map = Map::from_bytes(&mut content[..], elf_power);

    let num_elves = map.count_elves();
    let winner = run_once(&mut map);
    if winner == b'E' && map.count_elves() == num_elves { break; }
    elf_power += 1;
  }
...
impl<'a> Map<'a> {
  ...
  fn from_bytes(content: &mut [u8], elf_power: u32) -> Map {
    ...
    for (y, row) in rows.iter().enumerate() {
      for (x, unit) in row.iter().enumerate() {
        match unit {
          b'E' => { units.insert((x, y), Unit::init(elf_power)); }
          b'G' => { units.insert((x, y), Unit::init(3)); }
          _ => ()
        }
      }
    }
...
impl Unit {
  pub fn init(power: u32) -> Unit { Unit { power, hp: 200 } }
}
